...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.UniqueConstraint("organization_id", "provider", name="uq_org_payment_config_provider"),
    )
    op.create_index(
    ...
    # Транзакции оплаты (аудит + статусы + ссылки)
    op.create_table(
    ...
    op.create_index("ix_payment_tx_order_id", "payment_transactions", ["order_id"])
    op.create_index(
