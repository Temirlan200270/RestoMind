...
Подробный список изменений и возможностей — в [CHANGELOG.md](CHANGELOG.md). План развития (фазы 6–10, RAG, iiko delivery) — в [plan.md](plan.md).

## Возможности

- **Приём заказов** — распознавание блюд, сумма, подтверждение перед отправкой в iiko
...
| Тесты | pytest, pytest-asyncio (`tests/`, ~20 тестов) |
...
**Автоменю:** при запуске `uvicorn`, если таблица `menu_items` пуста, меню подставляется из встроенного каталога (`menu_bootstrap`) — отдельный seed не обязателен. Правки — во вкладке **«Меню»** админки.

В админке кнопка **«Демо-данные»** добавляет пользователей с префиксом `demo7700…` и **не стирает** остальное; при пустом меню подтягивается тот же каталог.
