...
def test_session_pooler_caps_explicit_pool_size():
    url = "postgresql://postgres.x:pass@aws-0-eu.pooler.supabase.com:5432/postgres"
    cfg = resolve_postgres_pool_settings(url, pool_size=20, max_overflow=10)
    assert cfg.pool_size == 1
    assert cfg.max_overflow == 0


def test_explicit_pool_size_overrides_generic_auto():
