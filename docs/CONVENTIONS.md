...
8. **Нет айтишных терминов в UI** — пользовательский интерфейс говорит языком оператора ресторана, не разработчика. Запрещено добавлять в UI-тексты, лейблы, кнопки и подсказки: `webhook`, `payload`, `endpoint`, `API`, `boolean`, `null`, `callback`, `UUID`, `JSON`, `migrate`, `deprecated` и подобные технические термины, **если их там не было раньше**. Исключение: раздел «Настройки» для технических интеграций (iiko, WhatsApp), где термины неизбежны и уже присутствуют.
