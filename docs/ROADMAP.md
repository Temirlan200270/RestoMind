...
- [ ] **Control Plane — timeline UI panel:** admin UI по `trace_id` (API `GET /trace-timeline` ✅); Phase 3 replay harness — см. CONTROL_PLANE.
...
- [ ] **SupplyMind checklist item PATCH:** persist checked state per draft item (`PATCH …/drafts/{id}` body `items[]`).
