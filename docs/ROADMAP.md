...
- [ ] **BI по iiko:** продажи по времени суток и автоподстройка upsell. _Wishlist Темира #16._ **MVP ✅:** `sales_by_hour_local` + `sales_insights.peak_hours_local` / upsell hint в `GET /analytics` (данные Order, не iiko ETL); полный iiko BI — backlog.
...
- [x] **Sprint A — Bot perf:** `schedule_save_ai_context_snapshot` (fire-and-forget до LLM); typing indicator / fast ack после 2с; heuristic model routing (`AI_MODEL_ROUTING_ENABLED`, fast→strong rerun).
- [x] **Sprint B — Admin lazy load:** conditional `loadChatList`; `loadRevenueLeak`/`loadShiftState` только dashboard/shift; login prefetch текущей вкладки + `deferIdleWork`; ETag/304 на profile + integrations/status.
- [x] **Rule 8 — UI operator language:** убран dev-жаргон с дашборда (очередь задач, DRAFT/AOV), «эскалация» → «запрос оператору», полный `operational_label` в шапке, лояльность без env-имён; технические статусы — только в настройках интеграций/состояния. Тест: [`tests/test_ui_operator_language.py`](tests/test_ui_operator_language.py).
