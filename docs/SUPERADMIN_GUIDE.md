...
- Сгенерированный пароль показывается **один раз** в UI (toast). Его легко потерять.
...
Сейчас `generated_password` показывается в toast.

- **Сделать**: после approve/create показывать модалку:
  - email
  - пароль
  - кнопки “Скопировать”
  - чекбокс “Я сохранил пароль”
- **Сделать**: серверный одноразовый “секрет” или “reset link” вместо показа пароля (как более безопасный контур).
