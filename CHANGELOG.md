...
- **Админка / E5 (тонкий UI):** строка статуса очереди задач на дашборде (обзор) и в «Настройки → Подключения»; клиент ходит на `GET /api/admin/system/task-queue-health` (если эндпоинта ещё нет — бейдж «недоступно», без падений). TODO в JS для выравнивания контракта с бэкендом.
