...
- **Демо из админки (`POST /api/admin/demo/seed`):** **10** демо-клиентов, **~48** заказов (13 сценарных + **35** объёмных по 7 дням UTC для графика), **14** броней, **~30** сообщений в `chat_logs` с растянутыми `created_at`; если **меню пустое**, подгружается полный список из `PLOVXANA_MENU_ITEMS` (~137 позиций). Повторный seed при уже существующих демо-клиентах: **409** как раньше, но если меню было пустым — **200** с `partial` и добавлением меню.
